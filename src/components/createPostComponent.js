            ...
            <form id="createPostForm">
                <div class="form-group">
                    <div class="cover-image-container" id="coverImageContainer" title="Click or drag image to add cover image">
                        ...
                        <img id="coverImagePreview" class="cover-image-preview" src="${
                          postData.coverImage || ""
                        }" alt="" style="display: ${
    postData.coverImage ? "block" : "none"
  };">
                        <button type="button" class="remove-cover-image-btn" id="removeCoverImage" style="display: none;">×</button>
                    </div>
                </div>
                <div class="form-group">
                    <input type="text" id="title" name="title" placeholder="Post Title" required value="${
                      postData.title || ""
                    }">
                </div>
                <div class="form-group">
                    ...
                    <select id="category" name="category" required>
                        ...
                        <option value="technology" ${
                          postData.category === "technology" ? "selected" : ""
                        }>Technology</option>
                        <option value="lifestyle" ${
                          postData.category === "lifestyle" ? "selected" : ""
                        }>Lifestyle</option>
                        <option value="travel" ${
                          postData.category === "travel" ? "selected" : ""
                        }>Travel</option>
                        <option value="food" ${
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" id="tags-hidden" name="tags" value='${JSON.stringify(
                      postData.tags || []
                    )}'>
                </div>
                <div class="form-group">
                    ...
                    <div id="editor" contenteditable="true" class="form-group input">${
                      postData.content || ""
                    }</div>
                    <textarea id="content" name="content" style="position: absolute; opacity: 0; pointer-events: none;">${
                      postData.content || ""
                    }</textarea>
                </div>
    ...
    if (form) {
      form.addEventListener("submit", async function (e) {
        ...
        try {
          ...
          const coverImageSrc =
            coverImage.style.display !== "none" ? coverImage.src : "";

          // Compress cover image if exists
          const optimizedCoverImage = coverImageSrc
            ? await optimizeImageData(coverImageSrc)
            : "";
          ...
          // Update display
          document.getElementById("createPostFormContainer").style.display =
            "none";
          ...
          alert(
            "Error saving post. Please try again with less content or smaller images."
          );
        }
      });
    }
      ...
      reader.onload = async (event) => {
        try {
          ...
          uploadedImageDiv.innerHTML += `
                    ...
                    `;
          document
            .getElementById("uploadedImagesList")
            .appendChild(uploadedImageDiv);
